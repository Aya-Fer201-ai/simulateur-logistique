...
def generate_list_from_config(df, config_tuple):
    if not isinstance(config_tuple, tuple) or len(config_tuple) != 2: return df.index.tolist()
    ...
    if sort_column not in df_copy.columns: return df_copy.index.tolist()
    return df_copy.sort_values(by=sort_column, ascending=ascending_order).index.tolist()

def create_excel_download(sim_results):
    output = BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        if sim_results:
            pd.DataFrame([{"Profit": sim_results.get('profit', 0), "Jours": sim_results.get('days_taken_simulation_loop', 'N/A'), "Demande_Satisfaite": sim_results.get('all_demand_met', False)}]).to_excel(writer, sheet_name="Resume", index=False)
            if sim_results.get('shipments_df') is not None: sim_results.get('shipments_df').to_excel(writer, sheet_name="Expeditions_Detail", index=False)
            if sim_results.get('final_destinations_df') is not None: sim_results.get('final_destinations_df').to_excel(writer, sheet_name="Recap_Destinations")
            if sim_results.get('final_origins_df') is not None: sim_results.get('final_origins_df').to_excel(writer, sheet_name="Recap_Origines")
            if sim_results.get('final_tracking_vars', {}).get('daily_wagon_log'): pd.DataFrame(sim_results.get('final_tracking_vars').get('daily_wagon_log')).to_excel(writer, sheet_name="Suivi_Wagons_Quotidien", index=False)
...
if 'log_optim' not in st.session_state: st.session_state.log_optim = ""
...
if relations_file and origins_file and destinations_file:
    ...
    if relations_df is not None:
        ...
        if mode_choice == "Simulation Simple":
            ...
            max_iter = st.sidebar.number_input("Nombre max d'itérations", 1, 100, 10, help="Pour des tests rapides, utilisez 1 ou 2.")
        ...
        if st.sidebar.button("🚀 Lancer l'Exécution", use_container_width=True):
            ...
            with st.spinner("Exécution en cours..."):
                try:
                    ...
                    st.error(f"Une erreur est survenue pendant l'exécution de la simulation: {e}")
                    import traceback
...
if st.session_state.results:
    ...
    if st.session_state.log_optim:
        with st.expander("Voir le journal de l'optimisation", expanded=False): st.code(st.session_state.log_optim, language="bash")
    col1, col2, col3 = st.columns(3)
    col1.metric("Profit Final (Tonnes * km)", f"{res.get('profit', 0):,.0f}".replace(',', ' ')); col2.metric("Jours de simulation", f"{res.get('days_taken_simulation_loop', 'N/A')}"); col3.metric("Demande satisfaite ?", "✅ Oui" if res.get('all_demand_met', False) else "❌ Non")
    excel_data = create_excel_download(res)
    ...
    shipments_df = res.get('shipments_df'); final_dest_df = res.get('final_destinations_df'); final_orig_df = res.get('final_origins_df')
    ...
    with tab_graph:
        ...
        if shipments_df is not None and not shipments_df.empty:
            ...
            with col1_graph:
                st.write("**Quantité totale livrée par destination**"); st.bar_chart(shipments_df.groupby('destination')['quantity_tons'].sum().sort_values(ascending=False))
                st.write("**Quantité totale expédiée par origine**"); st.bar_chart(shipments_df.groupby('origin')['quantity_tons'].sum().sort_values(ascending=False))
            with col2_graph:
                ...
                if final_dest_df is not None and 'annual_demand_tons' in final_dest_df.columns and 'delivered_so_far_tons' in final_dest_df.columns:
                    recap_df = final_dest_df.copy(); recap_df['satisfaction_rate'] = recap_df.apply(lambda row: (row['delivered_so_far_tons'] / row['annual_demand_tons'] * 100) if row['annual_demand_tons'] > 0 else 0, axis=1).fillna(0); st.bar_chart(recap_df['satisfaction_rate'])
                else: st.warning("Données manquantes pour le graphique de satisfaction.")
            st.write("**Flux d'expédition par jour (en tonnes)**"); st.line_chart(shipments_df.groupby('ship_day')['quantity_tons'].sum())
        else: st.info("Aucune expédition n'a été réalisée.")
    with tab_transport:
        st.subheader("Détail des Expéditions (Tableau de Transport)")
        if shipments_df is not None and not shipments_df.empty: st.dataframe(shipments_df.style.format(precision=2))
        else: st.info("Aucune expédition réalisée.")
    with tab_dest:
        st.subheader("État Final par Destination")
        if final_dest_df is not None: st.dataframe(final_dest_df.style.format(precision=2))
        else: st.info("Aucun résultat pour les destinations.")
    with tab_orig:
        st.subheader("État Final par Origine")
        if final_orig_df is not None: st.dataframe(final_orig_df.style.format(precision=2))
        else: st.info("Aucun résultat pour les origines.")
    with tab_wagon:
        ...
        if tracking_vars:
            ...
            if not wagon_log_df.empty:
                ...
                with st.expander("Voir les données détaillées du suivi"): st.dataframe(wagon_log_df)
            else: st.info("Aucun suivi de wagon enregistré.")
